...
public class LoginWindow extends Window{
	...
	LoginWindow(Orbit orbit){
		...
		status = new JLabel("Connect to a server lobby then login.");
		...
		guestLoginButton.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e) {
				if(connected){
					...
					for(int i = 0 ; i < 50; i++){
						int random = (int) (System.currentTimeMillis() % 100000); 
						username = "n00b"+random;
						password = "n00b"+random;
					...
					status.setText("You are not connected to a server");
				}
			}
		});
		...
		newUserButton.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e) {
				if(connected){
					...
					if(username.equals("") || password.equals("")){
						//report error
						status.setText("Invalid username and password combination.");
						System.out.println("Invalid username and password combination.");
					}
					else{
						System.out.println("Creating new user: username: " + username + ", password: " + password);
						if(createUser(username, password)){
							status.setText("New user created! username: " + username + ", password: " + password);
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(ip.equals("")){
					...
					if(Orbit.initializeSocket()){
						...
						status.setText("Connected Sucessfully");
					}
				}
			}
		});
	...
	public boolean createUser(String username, String password){ //adds username and password to databaseb
		Vector<String> strings = new Vector<String>();
		strings.add(username);
		strings.add(password);
