...
public class OrbitServerThread extends Thread {
	...
	public void run(){
		...
		try {
			...
			//IMPORTANT only ServerRequest objects are received by server
			ServerRequest sr = (ServerRequest)ois.readObject();
			try {
				//take appropriate action per request
				fulfillRequest(sr);
				...
				//send null object to indicate server request was not fulfilled
				sendResponse(null);
			}
		} catch (IOException e) {
